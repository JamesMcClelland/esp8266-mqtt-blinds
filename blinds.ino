...
SavedObject readData; //Variable to store custom object read from EEPROM.

/*
 * speed -> amount 0-100
 * limit -> amount 0 or 100
 * turn -> amount -0.5 - +0.5
 * mqtt -> clientID server username password
 */
...
/*
 * URL: speed ->  Params: amount 0-100
 * URL: limit -> Params: amount 0 or 100
 * URL: turn -> Params: amount -0.5 - +0.5
 * URL: mqtt -> Params: clientID server username password
 */

//Todo: create generic save function so individual arguemnts can be changed and saved.
...
void setup() {
    ...
    EEPROM.get( dataAddress, customVar );
    if (!customVar.initialised) {
